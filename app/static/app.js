const state = {
  file: null, tmpDir: null, cows: [],
  currentCowIndex: 0, farmRecords: {}, validPairs: []
}
...
dropZone.addEventListener('drop', e => {
  ...
  else if (f) alert('Please drop a .zip file.')
})
...
function updateRecordsNav() {
  ...
  document.getElementById('next-cow').textContent =
    state.currentCowIndex === state.cows.length - 1 ? 'Done ✓' : 'Next →'
}

document.getElementById('prev-cow').addEventListener('click', () => {
  saveCurrentCowRecords(); state.currentCowIndex--
  renderCowForm(state.currentCowIndex); updateRecordsNav()
})
document.getElementById('next-cow').addEventListener('click', () => {
  saveCurrentCowRecords()
  if (state.currentCowIndex < state.cows.length - 1) {
    state.currentCowIndex++; renderCowForm(state.currentCowIndex); updateRecordsNav()
  } else finishRecords()
})
...
async function finishRecords() {
  ...
  if (recordsToUpdate.length > 0) {
    try {
      const res = await fetch('/api/update_records', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tmp_dir: state.tmpDir, records: recordsToUpdate })
      })
      const data = await res.json()
  ...
  setProgress(60, 'Computing all breeding pairs...')
  try {
    const res = await fetch('/api/breeding_pairs', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ cows: state.cows, top_n: 9999 })
    })
    const data = await res.json()
...
function buildPairsTable(pairs) {
  ...
  const allCowIds = new Set(state.cows.map(c => c.id))
  const coveredIds = new Set()
  pairs.forEach(p => { coveredIds.add(p.cow1.id); coveredIds.add(p.cow2.id) })
  const allCovered = [...allCowIds].every(id => coveredIds.has(id))

  document.getElementById('pairs-note').innerHTML =
    `${pairs.length} valid Dam × Sire pairs shown` +
    (allCovered
      ? ` — <strong style="color:var(--green-mid)">✓ all ${state.cows.length} cows appear in the recommendations below</strong>`
      : ` — <strong style="color:var(--red)">⚠ some cows have no valid pairing (check sex assignments)</strong>`)
}
...
function buildBreedingTreeSVG(pairs) {
  ...
  <text x="20" y="30" font-size="13" font-weight="600" fill="#1a1a1a">Top ${pairs.length} Dam × Sire pairings</text>`
  ...
  pairs.forEach((pair, i) => {
    ...
    const box = (score) => ({
      fill:   score >= 60 ? '#dff0e5' : score >= 40 ? '#fff3dc' : '#fbe3e3',
      border: score >= 60 ? '#3b6d11' : score >= 40 ? '#ba7517' : '#a32d2d',
      text:   score >= 60 ? '#1a5c2e' : score >= 40 ? '#7a4a00' : '#7a1a1a'
    })
    const d = box(dam.score), s = box(sire.score), o = box(pair.offspring_score)
    svg += `
    ...
    <text x="156" y="${y+28}" text-anchor="middle" font-size="12" font-weight="600" fill="#1a1a1a">${dam.filename.length>22?dam.filename.substring(0,20)+'…':dam.filename}</text>
    <text x="156" y="${y+46}" text-anchor="middle" font-size="11" fill="#666">♀ ${dam.score}% low-risk</text>
    <text x="270" y="${y+38}" text-anchor="middle" font-size="18" fill="#999">×</text>
    <rect x="284" y="${y+8}" width="200" height="52" rx="8" fill="${s.fill}" stroke="${s.border}" stroke-width="1"/>
    <text x="384" y="${y+28}" text-anchor="middle" font-size="12" font-weight="600" fill="#1a1a1a">${sire.filename.length>22?sire.filename.substring(0,20)+'…':sire.filename}</text>
    <text x="384" y="${y+46}" text-anchor="middle" font-size="11" fill="#666">♂ ${sire.score}% low-risk</text>
    ...
    <text x="644" y="${y+26}" text-anchor="middle" font-size="11" fill="#666">Predicted offspring</text>
    <text x="644" y="${y+46}" text-anchor="middle" font-size="15" font-weight="700" fill="${o.text}">${pair.offspring_score}% low-risk</text>`
  })
  svg += `</svg>`
...
document.getElementById('download-report').addEventListener('click', async () => {
  ...
  try {
    const res = await fetch('/api/report', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ cows: state.cows, pairs: state.validPairs })
    })
